...
const{ User, Product, Books, Catalog} = require('../db');
...
const createBooks = async ({ Book_Title, Author_name, Year_of_Publication, Library_name, Book_Language, ISBN_No, category, No_of_pages }) => {
    return await Books.create({ Book_Title, Author_name, Year_of_Publication, Library_name, Book_Language, ISBN_No, category, No_of_pages });
};
...
const createLibraries = async ({ Library_name, Address, Timing, Website }) => {
    return await Libraries.create({ Library_name, Address, Timing, Website });
};
...
const createCatalog = async ({ Book_Title, Author_name, Year_of_Publication, Library_name, Book_Language, ISBN_No, category, No_of_pages }) => {
    return await Catalog.create({ Book_Title, Author_name, Year_of_Publication, Library_name, Book_Language, ISBN_No, category, No_of_pages });
};
...
getAllLibraries().then(libraries => res.json(libraries));
});


app.get('/catalog', function(req, res) {
  getAllCatalog().then(etc => res.json(etc));
});
